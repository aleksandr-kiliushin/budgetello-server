...
psql -U postgres -c "CREATE DATABASE budgetello_db ENCODING 'UTF-8';";

npm --prefix /var/app run migration:up;

# Seed database with testing data.
psql budgetello_db postgres << EOF
...
# Create testing database template.
psql -U postgres -c "DROP DATABASE IF EXISTS budgetello_testing_template WITH (FORCE);";
psql -U postgres -c "CREATE DATABASE budgetello_testing_template WITH TEMPLATE budgetello_db ENCODING 'UTF-8';";
