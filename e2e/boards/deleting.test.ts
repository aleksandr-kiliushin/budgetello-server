...
describe("Board deleting", () => {
  it("returns a correct response after deleting", async () => {
    ...
    const response = await fetchApi(`/api/boards/${boards.cleverFinanciers.id}`, { method: "DELETE" })
  ...
  it("returns a correct response after deleting", async () => {
    ...
    expect(await response.json()).toEqual(boards.cleverFinanciers)
  })

  it("the deleted board is not presented in all categories list", async () => {
    await authorize(users.johnDoe.username)
    await fetchApi(`/api/boards/${boards.cleverFinanciers.id}`, { method: "DELETE" })
