import { budgetingCategories, budgetingRecords } from "#e2e/constants/budgeting"
...
describe("Budgeting record updating", () => {
  test.each<{
    ...
    },
    {
      ...
      response: {
        amount: 100,
        category: budgetingCategories.educationExpense,
        date: "2022-08-01",
        id: budgetingRecords["1st"].id,
        isTrashed: true,
      },
  ...
  ])("Budgeting record updating case #%#", async ({ payload, response, status }) => {
    const recordUpdatingResponse = await fetchApi(`/api/budgeting/records/${budgetingRecords["1st"].id}`, {
